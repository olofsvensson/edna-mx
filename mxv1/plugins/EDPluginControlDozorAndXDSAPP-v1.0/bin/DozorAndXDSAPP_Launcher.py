...
logger = logging.getLogger("XDSAPP_Launcher")
...
XDSAPPPath = os.path.join(autoprocessingPath, "XDSAPP")
if not os.path.exists(XDSAPPPath):
    os.makedirs(XDSAPPPath, 0o755)
xdsInputFile = os.path.join(autoprocessingPath, "XDS.INP")

# Wait for XDS input file
xdsAppeared = False
waitXdsStart = time.time()
logger.info("XDSAPP launcher: waiting for XDS.INP file")
while not xdsAppeared and (time.time() - waitXdsStart < WAIT_XDS_TIMEOUT):
    ...
    if os.path.exists(xdsInputFile) and os.stat(xdsInputFile).st_size > 0:
        time.sleep(1)
        xdsAppeared = True
        logger.info("XDSAPP launcher: XDS.INP file is there, size={0}".format(os.stat(xdsInputFile).st_size))
if not xdsAppeared:
...
<XSDataInputControlDozorAndXDSAPP>
  <dataCollectionId>
    <value>{dataCollectionId}</value>
  </dataCollectionId>
  <processDirectory>
    <path>
      <value>{XDSAPPPath}</value>
    </path>
  </processDirectory>
</XSDataInputControlDozorAndXDSAPP>
"""
...
inputXml = inputTemplate.format(dataCollectionId=dataCollectionId,
                                XDSAPPPath=XDSAPPPath)

# we now need a temp file in the data dir to write the data model to
ednaInputFileName = "XDSAPP_input.xml"
ednaInputFilePath = os.path.join(XDSAPPPath, ednaInputFileName)
if os.path.exists(ednaInputFilePath):
    ...
    ednaInputFile = tempfile.NamedTemporaryFile(suffix=".xml",
                                                ...
                                                dir=XDSAPPPath,
                                                delete=False)
    ednaInputFilePath = os.path.join(XDSAPPPath, ednaInputFile.name)
...
XDSAPPDirectory = "$XDSAPPDirectory"
inputFile = "$inputFile"

pluginName = "EDPluginControlDozorAndXDSAPPv1_0"
...
baseName = "{0}_XDSAPP".format(timeString)
...
linkName = "{hostname}_{date}-{time}".format(hostname=hostname,
                                             date=dateString,
                                             time=timeString)
os.symlink(baseDir, os.path.join(XDSAPPDirectory, linkName))

ednaLogName = "XDSAPP_{0}-{1}.log".format(dateString, timeString)
EDVerbose.setLogFileName(os.path.join(XDSAPPDirectory, ednaLogName))
...
ednaScriptFileName = "XDSAPP_launcher.sh"
ednaScriptFilePath = os.path.join(XDSAPPPath, ednaScriptFileName)
if os.path.exists(ednaScriptFilePath):
    # Create unique file name
    ednaScriptFile = tempfile.NamedTemporaryFile(suffix=".sh",
                                                 prefix="XDSAPP_launcher-",
                                                 dir=XDSAPPPath,
                                                 delete=False)
    ednaScriptFilePath = os.path.join(XDSAPPPath, ednaScriptFile.name)
...
while not submitSuccess and len(remainingBesHosts) > 0:
    ...
    try:
        ...
        params = urllib.urlencode({"ednaDpLaunchPath":os.path.join(XDSAPPPath, ednaScriptFilePath),
                                   "beamline": beamline,
                                   "proposal": proposal,
                                   "initiator": beamline,
                                   ...
                                   "reuseCase": "true" })
        conn.request("POST",
                     "/BES/bridge/rest/processes/XDSAPP/RUN?%s" % params,
