...
class EDPluginControlStrategyv1_3(EDPluginControl):
        ...
        if not self.isFailure():
            ...
            if (self._edPluginRaddose is not None):
                ...
                if(xsDataStringSpaceGroup is not None):
                    ...
                    if strSpaceGroup != "":
                        ...
                        try:
                            ...
                            strErrorMessage = EDMessage.ERROR_EXECUTION_03 % ('EDPluginControlStrategyv1_3.preProcess', "Problem to calculate Number of symmetry operators", detail)
                            self.error(strErrorMessage)
                            self.addErrorMessage(strErrorMessage)
                ...
                if (strNumOperators is not None):
                    ...
                    strErrorMessage = EDMessage.ERROR_EXECUTION_03 % ('EDPluginControlStrategyv1_3.preProcess', "No symmetry operators found for Space Group: ", strNumOperators)
                    self.error(strErrorMessage)
                    self.addErrorMessage(strErrorMessage)
                    raise RuntimeError, strErrorMessage
    ...
    def configure(self):
        ...
        self.DEBUG("EDPluginControlStrategyv1_3.configure")
        strSymopHome = self.config.get(self._strCONF_SYMOP_HOME)
        if strSymopHome is None:
            strWarningMessage = EDMessage.WARNING_NO_PARAM_CONFIGURATION_ITEM_FOUND_03 % ('EDPluginControlStrategyv1_3.configure', self._strCONF_SYMOP_HOME, self.getPluginName())
            self.warning(strWarningMessage)
            self.addWarningMessage(strWarningMessage)
        else:
    ...
    def doFailureActionRaddose(self, _edPlugin=None):
        ...
        strWarningMessage = EDMessage.WARNING_CANNOT_USE_PLUGIN_03 % ('EDPluginControlStrategyv1_3.doFailureActionRaddose', self._strPluginRaddoseName, "Raddose failure")
        self.warning(strWarningMessage)
        self.addWarningMessage(strWarningMessage)
