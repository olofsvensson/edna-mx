...
class EDPluginControlXia2DIALSv1_0(EDPluginControl):
    ...
    def checkParameters(self):
        ...
        self.checkMandatoryParameters(
            self.dataInput.dataCollectionId, "No data collection id"
        )
    ...
    def preProcess(self, _edObject=None):
        ...
        if self.doNoanom:
            self.edPluginExecXia2DIALSNoanom = self.loadPlugin(
                "EDPluginExecXia2DIALSv1_0", "EDPluginExecXia2DIALSv1_0_noanom"
            )

        if self.dataInput.icatProcessDataDir is not None:
            self.icatProcessDataDir = self.dataInput.icatProcessDataDir.path.value
    ...
    def process(self, _edObject=None):
        ...
        if self.doAnom:
            (
                ...
                self.hasUploadedAnomResultsToISPyB,
            ) = self.uploadToISPyB(
                self.edPluginExecXia2DIALSAnom,
                True,
                proposal,
                self.autoProcProgramIdAnom,
                self.autoProcIntegrationIdAnom,
            ...
            # Upload to ICAT
            if xsDataInputStoreAutoProc is not None and self.icatProcessDataDir is not None:
                EDUtilsICAT.uploadToICAT(
                    processName="XIA2_DIALS",
                    xsDataInputStoreAutoProc=xsDataInputStoreAutoProc,
                    directory=directory,
                    icatProcessDataDir=self.icatProcessDataDir,
                    isAnom=True,
                    beamline=beamline,
                    proposal=proposal,
                    timeStart=None,
        ...
        if self.doNoanom:
            (
                xsDataInputStoreAutoProc,
                self.hasUploadedNoanomResultsToISPyB,
            ) = self.uploadToISPyB(
                self.edPluginExecXia2DIALSNoanom,
                False,
                proposal,
                self.autoProcProgramIdNoanom,
                self.autoProcIntegrationIdNoanom,
    ...
    def uploadToISPyB(
        self, edPluginExecXia2DIALS, isAnom, proposal, programId, integrationId
    ):
        successUpload = False
        ...
        if (
            self.pyarchDirectory is not None
            and edPluginExecXia2DIALS.dataOutput.ispybXML is not None
        ):
            ...
            if edPluginExecXia2DIALS.dataOutput.logFile is not None:
                ...
                shutil.copy(
                    pathToLogFile, os.path.join(self.pyarchDirectory, pyarchFileName)
                )
                shutil.copy(
                    pathToLogFile, os.path.join(self.resultsDirectory, pyarchFileName)
                )
            ...
            if edPluginExecXia2DIALS.dataOutput.summary is not None:
                ...
                pyarchFileName = (
                    self.pyarchPrefix + "_" + anomString + "_xia2-summary.log"
                )
                shutil.copy(
                    pathToSummaryFile,
            ...
            if edPluginExecXia2DIALS.dataOutput.htmlFile is not None:
                ...
                shutil.copy(
                    pathToHtmlFile, os.path.join(self.pyarchDirectory, pyarchFileName)
                )
                shutil.copy(
                    pathToHtmlFile, os.path.join(self.resultsDirectory, pyarchFileName)
                )
                autoProcProgramAttachment = AutoProcProgramAttachment()
                autoProcProgramAttachment.fileName = pyarchFileName
                autoProcProgramAttachment.filePath = self.pyarchDirectory
        ...
        return xsDataInputStoreAutoProc, successUpload
