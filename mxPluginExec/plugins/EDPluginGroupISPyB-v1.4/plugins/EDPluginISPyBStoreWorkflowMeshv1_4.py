...
class EDPluginISPyBStoreWorkflowMeshv1_4(EDPluginISPyBv1_4):
    ...
    def process(self, _edObject=None):
        ...
        cartographyPath = self.getXSValue(
            xsDataInputWorkflowMesh.cartographyPath, _bDoTruncate=False
        )
        # DPC-152 : Make sure that file path is never more than 255 characters
        cartographyPath = EDUtilsPath.truncateFilePath(cartographyPath)
        workflowMeshWS3VO.cartographyPath = cartographyPath
        #        print workflowMeshWS3VO
        self.iWorkflowMeshId = (
            clientToolsForCollectionWebService.service.storeOrUpdateWorkflowMesh(
                workflowMeshWS3VO
            )
        )
        self.DEBUG(
    ...
    def finallyProcess(self, _edObject=None):
        ...
        if self.iWorkflowMeshId is not None:
            xsDataResultISPyBStoreWorkflowMesh.workflowMeshId = XSDataInteger(
                self.iWorkflowMeshId
            )
